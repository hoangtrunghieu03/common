...
class WebSocketHelper {
    ...
    /**
         ...
         * @param roomId 
         */
    public sendNotification(data: any, roomId: string) {
        this.connections.forEach((connection) => {
            let sendData: any = {};
            sendData.token = roomId;
            sendData.type = "notification_Reload";
            sendData.data = data;
     ...
     * @param locationId 
     */
    public SendReloadOrder(data: any, storeId: string) {
        this.connections.forEach((connection) => {
            if (connection.connectionInfo.criteria === storeId) {
                let sendData: any = {};
                sendData.token = storeId;
    ...
    public SendReloadOrderDelivery(data: any, storeId: string) {
        this.connections.forEach((connection) => {
            if (connection.connectionInfo.criteria === storeId) {
                let sendData: any = {};
                sendData.token = storeId;
                sendData.type = "orderdelivery_Reload";
                sendData.data = data;
                let stringifiedData = JSON.stringify(sendData);
                connection.socket.send(stringifiedData);
            }
