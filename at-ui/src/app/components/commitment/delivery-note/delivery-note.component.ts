...
export class DeliveryNoteComponent implements OnInit {
  ...
  createForm = (): FormGroup => {
    return this._formBuilder.group({
      ...
      sender: [this.getStaffName(this.staffLoginId)], // Id người nhận mẫu
      receiver: [null], // Id người nhận khí cụ
      dateDelivery: [this.today], // ngày giao
  ...
  saveImage = (): void => {
    if (this.deliveryForm.controls.dateDelivery.errors?.ngbDate) {
    ...
    let timeOut = setTimeout(() => {
      ...
      .then((fileBase64) => {
        this.deliveryForm.get('image').setValue(fileBase64);
        this.deliveryForm.get('dateDelivery').setValue(this.formatDate(this.deliveryForm.get('dateDelivery').value));
        this.activeModal.close(this.deliveryForm.value);
      }).catch((error) => {
